...
//!
//! Calls the Anthropic REST API (POST /v1/messages) directly using reqwest.
//! Enforces UTF-8, handles timeouts, retries once on empty output, and
//! captures full provenance including token usage.
...
/// Adapter that calls the Anthropic Messages API over HTTPS.
...
impl AnthropicApiAdapter {
    ...
    /// Create with custom model and base URL.
    ...
    async fn invoke(&self, request: AdapterRequest) -> AdapterResponse {
            ...
            Ok(body) => {
                ...
                if normalized.result == NormalizationResult::Empty {
                    ...
                    tracing::warn!(
                        ...
                        task_id = %task_id,
                        "Empty output from Anthropic API, retrying once"
                    );
                    ...
                    if remaining.is_zero() {
                        ...
                        match self.call_once(&request, remaining).await {
                            Ok(body2) => {
                                let p2 = parse_anthropic_response(&body2);
        ...
        AdapterResponse {
            ...
            provenance: AdapterProvenance {
                adapter_name: "anthropic-api".to_string(),
                model_used,
...
fn parse_anthropic_response(body: &str) -> ParsedAnthropicResponse {
    ...
    ParsedAnthropicResponse { text, usage }
}
